...
function Combination(props) {
      ...
      <Row className="title-row" key={item.id}>
        ...
        <div className="traits">
          {item.trait.map((traitItem) => {
            return (
              <div className="card-column" key={traitItem.traitId}>
                <a
                  className="trait-link"
                  name={item.title}
                  href={traitItem.jpgStoreLink}
                >
                  <Card>
                    <Row>
                      {traitItem.image && (
        ...
        <Col className={item.backgroundImg} xs={{ size: 1 }}>
          <Container className="token-container">
            <Col xs={{ size: 12 }} className="token-row">
              <h3 className="title-text">{item.tokens + " $ETERNAL daily"}</h3>
            </Col>
            <Col xs={{ size: 12 }} className="token-row">
              <img
                className="token-image"
                alt="sin token"
                src="images/extras/token.png"
              ></img>
            </Col>
          </Container>
        </Col>
  ...
  return (
    <>
      <Container>{myCombination}</Container>
    </>
  );
}
